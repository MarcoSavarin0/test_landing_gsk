		...
		<Link href={`/blog/${slug}`} onClick={() => va.track(`Blog - Open ${slug} post`)}>
			<article className="group w-full h-auto sm:h-full rounded-xl shadow-md hover:shadow-xl active:shadow-md space-y-3 pb-4 bg-white hover:bg-gsk-orange transition ease-out duration-300 overflow-hidden">
				<div className="overflow-hidden flex">
					<Image src={`https://${process.env.NEXT_PUBLIC_AWS_BUCKET_NAME}.s3.${process.env.NEXT_PUBLIC_AWS_PUBLIC_REGION}.amazonaws.com/notas/${image}.webp`} className="w-full group-hover:scale-105 transition-transform ease-in-out duration-500" alt={title} width={1920} height={1080}/>
				</div>
